...
class login_page:
    def __init__(self, root):
        ...
        self.frame1 = Frame(self.window, bg="turquoise")
        self.frame1.place(x=0, y=0, width=450, relheight = 1)

        label1 = Label(self.frame1, text= "S", font=("times new roman", 40, "bold"), bg="turquoise", fg="red").place(x=100,y=300)
        label2 = Label(self.frame1, text= "SYS", font=("times new roman", 40, "bold"), bg="Purple", fg="RoyalBlue1").place(x=162,y=300)
        label3 = Label(self.frame1, text= "SMART SECURITY", font=("times new roman", 13, "bold"), bg="black", fg="white").place(x=100,y=360)
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                ...
                cur.execute("select * from employee_register where email=%s and password=%s",(self.email_entry.get(),self.password_entry.get()))
                row=cur.fetchone()
                if row == None:
                    messagebox.showerror("Error!","Invalid USERNAME & PASSWORD",parent=self.window)
                else:
                    messagebox.showinfo("Success","Welcome to APPLE WOOD SOCIETY",parent=self.window)
    ...
    def forgot_func(self):
        if self.email_entry.get()=="":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                cur = connection.cursor()
                cur.execute("select * from employee_register where email=%s", self.email_entry.get())
                row=cur.fetchone()
                if row == None:
    ...
    def change_pass(self):
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                cur = connection.cursor()
                cur.execute("select * from employee_register where email=%s and question=%s and answer=%s", (self.email_entry.get(),self.sec_ques.get(),self.ans.get()))
                row=cur.fetchone()
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("update employee_register set password=%s where email=%s", (self.new_pass.get(),self.email_entry.get()))
                        connection.commit()
